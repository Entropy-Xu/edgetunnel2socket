		...
		if (访问路径 === 'version' && url.searchParams.get('uuid') === userID) {// 版本信息接口
			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (访问路径 === 'admin/socks-adapter') {// 本地 SOCKS5 动态代理配置面板
						return new Response(生成SOCKS适配器面板(), { status: 200, headers: { 'Content-Type': 'text/html;charset=utf-8', 'Cache-Control': 'no-store' } });
					} else if (访问路径 === 'admin/socks-adapter/config') {// 生成 sing-box 动态代理配置
						return await 处理SOCKS适配器配置请求(request);
...
const SOCKS适配器最大输入字节 = 1024 * 1024;

function 生成SOCKS适配器面板() {
	return `<!doctype html>
<html lang="zh-CN">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>sing-box SOCKS5 适配器</title>
<style>
:root {
...
<main>
	<header>
		<div>
			<h1>sing-box SOCKS5 适配器</h1>
			<p>把 edgetunnel VLESS/WS/TLS 订阅转换为本地 sing-box 动态代理配置。</p>
		</div>
		<a class="link-button" href="/admin">返回后台</a>
	</header>
	<div class="grid">
		<section class="panel">
			<div class="field">
				<label for="sub">订阅 URL</label>
				<input id="sub" name="sub" placeholder="https://你的域名/sub?token=...">
			</div>
			<div class="field">
				<label for="input">订阅内容</label>
				<textarea id="input" name="input" spellcheck="false" placeholder="可以粘贴 base64 订阅或明文 vless:// 链接"></textarea>
			</div>
			<div class="row">
				<div class="field">
					<label for="listen">监听地址</label>
					<input id="listen" name="listen" value="127.0.0.1">
				</div>
				<div class="field">
					<label for="port">端口</label>
					<input id="port" name="port" value="1080" inputmode="numeric">
				</div>
			</div>
			<div class="actions">
				<button id="generate" type="button">生成配置</button>
				<button id="copy" class="secondary" type="button" disabled>复制</button>
				<button id="download" class="secondary" type="button" disabled>下载 JSON</button>
			</div>
			<div id="status" class="status"></div>
			<div class="commands">
				<div class="command">sing-box check -c sing-box.generated.json</div>
				<div class="command">sing-box run -c sing-box.generated.json</div>
				<div class="command">curl --socks5-hostname 127.0.0.1:1080 https://cloudflare.com/cdn-cgi/trace</div>
			</div>
		</section>
		<section>
			<pre id="output">生成后的 sing-box 配置会显示在这里。</pre>
		</section>
	</div>
</main>
<script>
...
function collectPayload() {
	return {
		sub: $('sub').value.trim(),
		input: $('input').value,
		listen: $('listen').value.trim() || '127.0.0.1',
		port: $('port').value.trim() || '1080',
	};
}
...
async function generate() {
	...
	setStatus('正在生成配置...');
	try {
		const response = await fetch('/admin/socks-adapter/config', {
			method: 'POST',
		...
		generatedConfig = JSON.stringify(data.config, null, 2);
		$('output').textContent = generatedConfig;
		$('copy').disabled = false;
		$('download').disabled = false;
		const warnings = data.warnings && data.warnings.length ? '\\n' + data.warnings.join('\\n') : '';
		setStatus('已生成 ' + data.nodes.length + ' 个 VLESS 节点。' + warnings, 'ok');
	} catch (error) {
		generatedConfig = '';
...
function downloadConfig() {
	...
	a.href = url;
	a.download = 'sing-box.generated.json';
	a.click();
...
async function 处理SOCKS适配器配置请求(request) {
	if (request.method !== 'POST') return SOCKS适配器错误响应('只支持 POST 请求', 405);
	try {
		const bodyText = await request.text();
		if (bodyText.length > SOCKS适配器最大输入字节) throw new Error('请求内容超过 1MiB 限制');
		const payload = bodyText ? JSON.parse(bodyText) : {};
		const sub = String(payload.sub || '').trim();
		const input = String(payload.input || '').trim();
		if (!sub && !input) throw new Error('请提供订阅 URL 或订阅内容');
		if (sub && input) throw new Error('订阅 URL 和订阅内容只能二选一');

		const listen = String(payload.listen || '127.0.0.1').trim() || '127.0.0.1';
		const port = 解析SOCKS适配器端口(payload.port || 1080);
		let content = input;
		if (sub) {
			const subURL = new URL(sub);
			if (!['http:', 'https:'].includes(subURL.protocol)) throw new Error('订阅 URL 只支持 http 或 https');
			const response = await fetch(subURL.href, {
				headers: {
					'User-Agent': 'edgetunnel-sing-box-panel/1.0',
					'Accept': 'text/plain, */*',
				},
			});
			if (!response.ok) throw new Error(`订阅请求失败: ${response.status} ${response.statusText}`);
			content = await response.text();
			if (content.length > SOCKS适配器最大输入字节) throw new Error('订阅内容超过 1MiB 限制');
		}

		const result = 生成SingBox动态代理配置(content, { listen, port });
		const rawConfig = JSON.stringify(result.config, null, 2);
		if (new URL(request.url).searchParams.has('raw')) {
			return new Response(rawConfig + '\n', {
				status: 200,
				headers: {
					'Content-Type': 'application/json;charset=utf-8',
					'Content-Disposition': 'attachment; filename="sing-box.generated.json"',
					'Cache-Control': 'no-store',
				},
			});
		}
		...
		return SOCKS适配器错误响应(error.message || String(error), 400);
	}
}

function SOCKS适配器错误响应(error, status = 400) {
...
function 生成SingBox动态代理配置(content, options = {}) {
	const normalized = 识别并解码订阅内容(content);
	const extracted = 提取VLESS订阅链接(normalized.content);
	if (extracted.vlessLinks.length === 0) throw new Error('订阅内容中没有找到 vless:// 链接');

	const warnings = [];
	if (normalized.decodedBase64) warnings.push('输入内容已按 base64 订阅自动解码');
	if (extracted.ignoredProtocols.size > 0) warnings.push('已忽略非 VLESS 链接: ' + 描述忽略协议(extracted.ignoredProtocols));

	const nodes = extracted.vlessLinks.map((link, index) => 解析VLESS订阅链接(link, index));
	const config = 构建SingBox动态代理配置(nodes, options);
	return {
		decodedBase64: normalized.decodedBase64,
		warnings,
...
function 识别并解码订阅内容(content) {
	const text = 去除BOM(String(content || '')).trim();
	const compact = text.replace(/\s+/g, '');
	const decoded = 尝试Base64解码(compact);
	if (!包含协议链接(text) && decoded && 包含协议链接(decoded)) return { content: decoded.trim(), decodedBase64: true };
	return { content: text, decodedBase64: false };
}

function 去除BOM(value) {
	return value.charCodeAt(0) === 0xfeff ? value.slice(1) : value;
}

function 包含协议链接(value) {
	return /[a-z][a-z0-9+.-]*:\/\//i.test(value);
}

function 尝试Base64解码(value) {
...
function 提取VLESS订阅链接(content) {
	const vlessLinks = [], ignoredProtocols = new Map(), seen = new Set();
	...
	return { vlessLinks, ignoredProtocols };
}

function 解析VLESS订阅链接(link, index = 0) {
	let url;
	try {
		url = new URL(link);
	} catch (error) {
		throw new Error(`第 ${index + 1} 个 VLESS 链接无效: ${error.message}`);
	}
	const uuid = 安全URL解码(url.username);
	const server = 规范化SingBox服务器地址(安全URL解码(url.hostname));
	if (!uuid) throw new Error(`第 ${index + 1} 个 VLESS 链接缺少 uuid`);
	if (!server) throw new Error(`第 ${index + 1} 个 VLESS 链接缺少 server`);
	const search = url.searchParams;
	const security = (search.get('security') || 'none').toLowerCase();
	const type = (search.get('type') || 'tcp').toLowerCase();
	const server_port = url.port ? 解析SOCKS适配器端口(url.port) : (security === 'tls' ? 443 : 80);
	const host = 首个查询参数(search, ['host', 'authority']);
	const sni = 首个查询参数(search, ['sni', 'peer']);
	const path = 首个查询参数(search, ['path', 'serviceName']) || '/';
	const fp = search.get('fp') || search.get('fingerprint') || 'chrome';
	const remark = 解析订阅备注(url.hash, `${server}:${server_port}`);
	return { uuid, server, server_port, security, type, host, sni, path, fp, remark, raw: link };
}

function 规范化SingBox服务器地址(server) {
	return server.replace(/^\[/, '').replace(/\]$/, '');
}

function 首个查询参数(search, names) {
...
function 解析订阅备注(hash, fallback) {
	if (!hash) return fallback;
	const encoded = hash.startsWith('#') ? hash.slice(1) : hash;
	return 安全URL解码(encoded.replace(/\+/g, '%20')) || fallback;
}

function 安全URL解码(value) {
	try {
...
function 解析SOCKS适配器端口(value) {
	const port = Number(value);
	if (!Number.isInteger(port) || port < 1 || port > 65535) throw new Error(`端口无效: ${value}`);
	return port;
}

function 构建SingBox动态代理配置(nodes, options) {
	const tags = 生成唯一出站标签(nodes);
	const outbounds = nodes.map((node, index) => 构建VLESS出站(node, tags[index]));
...
function 生成唯一出站标签(nodes) {
	const used = new Map();
	return nodes.map((node, index) => {
		const base = 清理SingBox标签(node.remark || `${node.server}:${node.server_port}` || `vless-${index + 1}`) || `vless-${index + 1}`;
...
function 清理SingBox标签(value) {
	return value.replace(/[\u0000-\u001f\u007f]/g, '').replace(/\s+/g, ' ').trim().slice(0, 80);
}

function 描述忽略协议(ignoredProtocols) {
